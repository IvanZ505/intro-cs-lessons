...
//        wetwetProbability[1] = latitude;
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        int[] wetordry = new int[numberOfDays];
        ...
        for(int x = 1;x < wetordry.length; x++) {
            double randomNum = StdRandom.uniform();
            if(wetordry[x-1] == 1) {
                if(randomNum < wetwetProbability) {
                    wetordry[x] = 1;
                } else if (randomNum > wetwetProbability) {
                    wetordry[x] = 2;
                }
            } else if (wetordry[x-1] == 2) {
                if(randomNum < drywetProbability) {
                    wetordry[x] = 1;
                } else if (randomNum > drywetProbability) {
                    wetordry[x] = 2;
                }
            }
        }
